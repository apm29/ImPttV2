...
class SettingsFragment : BaseNestedFragment() {
    ...
    override fun setupViews(view: View, savedInstanceState: Bundle?) {
        ...
        launch {
            ...
            checkboxAutoPlay.isChecked = pttService.voiceOn
            checkboxAutoPlay.setOnCheckedChangeListener { _, isChecked ->
                pttService.voiceOn = isChecked
            }
            pttService.registerObserverWithLifecycle(this@SettingsFragment,object:PttObserver(){
                override fun onVoiceToggleChanged(on: Boolean) {
                    super.onVoiceToggleChanged(on)
                    checkboxAutoPlay.isChecked = on
                }
            })
        }
    }
